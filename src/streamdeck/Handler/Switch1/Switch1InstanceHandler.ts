...
import { toggleSwitch1 } from "../../../homematic/toggleSwitch1";
...
export class Switch1InstanceHandler extends BaseStreamDeckInstanceHandler<Switch1Settings> {
    ...
    constructor(streamdeck: StreamDeck, device: string | undefined, context: string | undefined, initialSettings: Switch1Settings | undefined) {
        super(streamdeck, device, context, initialSettings);

        this.logger = getLogger(`Switch1InstanceHandler-${device}-${context}`);
        this.getState();
    }
    ...
    protected async getState() {
        try {
            ...
            if (this.settings.selectedDeviceId == null) {
                this.logger.error("No switch1 control device configured. Cannot refresh data from HomeMatic.");
                return;
            }
    ...
    public async onKeyDown(instance: StreamDeck, message: KeyDownMessage<Switch1Settings>) {
        const settings = message.payload.settings;
        this.logger.info(`KeyDown for switch1 "${settings.selectedDeviceName}".`);

        const newState = await toggleSwitch1(instance, this.settings);
