...
export class RelayInstanceHandler extends BaseStreamDeckInstanceHandler<RelaySettings> {
    ...
    protected async getState() {
        try {
            ...
            if (this.settings.selectedDeviceId == null) {
                this.logger.error("No relay control device configured. Cannot refresh data from HomeMatic.");
                return;
            }
    ...
    public async onKeyDown(instance: StreamDeck, message: KeyDownMessage<RelaySettings>) {
        const settings = message.payload.settings;
        this.logger.info(`KeyDown for relay "${settings.selectedDeviceName}".`);

        const newState = await toggleRelay(instance, this.settings);
